...
func runScalabilityTest(test ScalabilityTest) TestResult {
	...
	// Step 1a: Setup LE parameters
	leParams, err := psi.SetupLEParameters(X_size)
	if err != nil {
		result.ErrorMessage = fmt.Sprintf("SetupLEParameters failed: %v", err)
		return result
	}

	// Step 1b: Build Merkle tree with all records (keys are small, fits in RAM)
	dbPath := fmt.Sprintf("test_%s.db", test.Name)
	db, err := sql.Open("sqlite3", dbPath)
	if err != nil {
		result.ErrorMessage = fmt.Sprintf("Open tree db failed: %v", err)
		return result
	}
	defer db.Close()
	...
	if err := initTreeDB(db, leParams.Layers); err != nil {
		log.Printf("warning: initTreeDB: %v\n", err)
	}
	...
	fmt.Println("       💾 Loading Merkle Tree into RAM...")
	memoryTree, err := LE.LoadTreeFromDB(db, leParams.Layers, leParams)
	if err != nil {
		result.ErrorMessage = fmt.Sprintf("Failed to load tree: %v", err)
		return result
	}
	fmt.Printf("       ✓ Tree loaded in %v\n", time.Since(initStart))

	// Free public keys (no longer needed after tree is built)
	publicKeys = nil
	runtime.GC()

	result.InitializationTime = time.Since(initStart)

	// Track RAM after tree build
	...
	ciphertexts := clientEncryptParallelSafe(clientHashes, pp, msg, leParams)
...
// clientEncryptParallelSafe encrypts client records with bounded concurrency
func clientEncryptParallelSafe(clientHashes []uint64, pp *matrix.Vector, msg *ring.Poly, le *LE.LE) []psi.Cxtx {
...
func extractCryptoParams(ctx *psi.ServerInitContext) CryptoParams {
	...
	if le != nil {
		layers = le.Layers
		numSlots = 1 << le.Layers
		ringDim = le.D
	}
	return CryptoParams{
		SecurityLevel: secMode,
		RingDimension: ringDim,
